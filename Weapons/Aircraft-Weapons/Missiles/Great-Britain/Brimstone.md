---
designation: Brimstone
common_name: Dual-Mode Precision Missile
nation: UK
type: Anti-Ship Missile
introduced: 2005
retired: Active
service_life: 2005-Present
tags: [anti-ship-missile, dual-mode-precision-missile, uk, radar-required, laser-guided]
completeness: partial
---

# Brimstone Dual-Mode Precision Missile

## Overview

**Nation**: UK  
**Category**: Anti-Ship Missile  
**Service Life**: 2005-Present  

## Physical Specifications

- **Weight**: 110.0 lbs  
- **Length**: 5.9 ft  
- **Diameter**: 7.0 inches  

## Guidance & Requirements

**Guidance Type**: Millimetric Wave + SAL  

**Requirements**:  
- Requires Radar  
- Requires Targeting Pod  
- Requires Laser Designator  
- Requires Databus  
- Requires Electrical Interface  
- Minimum Fire Control Generation: 3  

## Performance

- **Range**: 32.0 nm  
- **Warhead Weight**: 13.2 lbs  
- **Damage**: 95  
- **Accuracy Rating**: 10/10  
- **Reliability Rating**: 9/10  

## Compatible Aircraft

See aircraft database for compatibility information.  
This weapon can be carried by any aircraft with appropriate hardpoints and fire control generation.  

## Notes

OPERATIONAL: MBDA precision strike missile. Dual-mode mmW radar + semi-active laser. 60+ km range (Brimstone 2 from fixed wing). Sub-1m CEP. 6.3kg tandem shaped-charge warhead. Fire-and-forget or laser designation. Anti-armor, anti-boat, anti-vehicle. Tornado, Typhoon, F-35B. Exceptional accuracy.

