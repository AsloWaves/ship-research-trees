...
/^\| [0-9]/ {
    if (in_table) {
        ...
        # Skip if not naval aircraft entry
        if (tech_branch != "Ground Aircraft") next

        # Convert build days to months
        ...
        # Format output
        printf "| %s | %s | %s | %s | Ground Aircraft | %s | Aircraft | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s |\n", \
